...
const url = base.includes('/services/technical-seo')
  ? base
  : `${base.replace(/\/$/, '')}/services/technical-seo`
const res = await fetch(url, { headers: { 'User-Agent': 'SIT-Technical-SEO-QA/1.0' } })
...
const quickAnswerH2s = headings('h2').filter((h) => /what .*technical seo/i.test(h))
...
const out = {
  ...
  technicalSeoChecklistLink: uniqueInternal.includes('/blog/technical-seo-checklist-enterprise-websites'),
  faqSchemaCount,
...
writeFileSync(join(root, 'seo-audit', 'phase-7b-local-qa.json'), JSON.stringify(out, null, 2))
