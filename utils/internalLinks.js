...
import {
  getHoldCitySlugs,
  getHoldIndustrySlugs,
  getSitemapPublishMode,
  isHoldPath,
} from './sitemapWaveConfig.js'

function shouldDeprioritizeHoldLinks() {
  return getSitemapPublishMode() === 'indexed_waves'
}

function filterHoldHrefs(hrefs) {
  if (!shouldDeprioritizeHoldLinks()) return hrefs
  return hrefs.filter((href) => !isHoldPath(href))
}

function filterHoldIndustrySlugs(slugs) {
  if (!shouldDeprioritizeHoldLinks()) return slugs
  const holdSet = new Set(getHoldIndustrySlugs())
  return slugs.filter((slug) => !holdSet.has(slug))
}

function filterHoldCitySlugs(slugs) {
  if (!shouldDeprioritizeHoldLinks()) return slugs
  const holdSet = new Set(getHoldCitySlugs())
  return slugs.filter((slug) => !holdSet.has(slug))
}
...
const INDUSTRY_BLOG_SLUGS = {
  ...
  'accountants-seo': ['google-business-profile-optimization-guide'],
  'doctor-physician-seo': ['seo-healthcare-businesses-europe'],
  'deck-builder-seo': ['google-business-profile-optimization-guide'],
  'chiropractor-seo': ['seo-healthcare-businesses-europe'],
  'movers-and-moving-company-seo': ['google-business-profile-optimization-guide'],
  'plastic-surgery-seo': ['seo-healthcare-businesses-europe'],
  'flooring-company-seo': ['google-business-profile-optimization-guide'],
  'fence-installer-seo': ['google-business-profile-optimization-guide'],
  'garage-door-seo': ['google-business-profile-optimization-guide'],
  'janitorial-service-seo': ['google-business-profile-optimization-guide'],
  'roofers-seo': ['google-business-profile-optimization-guide'],
  'physiotherapy-seo': ['seo-healthcare-businesses-europe'],
  'pest-control-service-seo': ['google-business-profile-optimization-guide'],
  'orthodontist-seo': ['seo-healthcare-businesses-europe'],
  'dentist-seo': ['google-business-profile-optimization-guide'],
  'locksmith-service-seo': ['google-business-profile-optimization-guide'],
  'cpa-firm-seo': ['google-business-profile-optimization-guide'],
  'dry-cleaner-seo': ['google-business-profile-optimization-guide'],
  'waste-management-seo': ['google-business-profile-optimization-guide'],
}
...
function resolveHrefs(hrefs) {
  return dedupeLinks(filterHoldHrefs(hrefs).map((href) => linkFromHref(href)).filter(Boolean))
}
...
export function getIndustryInternalLinks(entry) {
  ...
  const peerSlugs = filterHoldIndustrySlugs(
    industryEntries
...
export function getServiceInternalLinks(serviceSlug) {
  ...
  const limit = getPageLinkLimit(sourcePath)
  const industrySlugs = filterHoldIndustrySlugs(
    SERVICE_INDUSTRY_LINKS[serviceSlug] || ['realtor-seo', 'dentist-seo', 'hvac-seo']
  )
    ...
    hubSlug === 'seo'
      ? resolveHrefs(
          filterHoldHrefs([
            ...
            '/industries',
            '/industries/plastic-surgery-seo',
...
export function getLocationInternalLinks(currentSlug) {
  ...
  const limit = getPageLinkLimit(sourcePath)
  const otherCities = filterHoldCitySlugs(
    LOCATION_CITIES.filter((c) => c.slug !== currentSlug)
      .slice(0, 5)
      .map((c) => c.slug)
  ).map((slug) => `/seo-services/${slug}`)
