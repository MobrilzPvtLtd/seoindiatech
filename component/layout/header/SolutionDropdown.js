...
const SolutionCard = ({ title, description, icon, iconBg, color, slug, index }) => {
  ...
  return (
    <Link href={`/solution/${slug}`}>
      <div
        className={`bg-card dark:bg-card p-6 rounded-xl shadow-md hover:shadow-xl transition-all duration-500 border border-border dark:border-border transform hover:-translate-y-1 relative overflow-hidden cursor-pointer h-full`}
      ...
      >
        ...
        <div className="flex items-center mb-4 relative z-10">
          ...
          <h3 className={`text-md font-semibold text-heading transition-colors ${isHovered ? 'text-primary dark:text-accent' : ''}`}>
            {title}
          </h3>
        </div>

        <p className="text-muted mb-0 relative z-10 text-sm">
          {description}
        </p>
      </div>
    </Link>
...
export default function SolutionDropdown() {
  const solutions = [
    { 
      ...
      iconBg: 'bg-secondary dark:bg-surface',
    ...
    { 
      ...
      iconBg: 'bg-green-500 dark:bg-surface',
    ...
    { 
      ...
      iconBg: 'bg-orange-500 dark:bg-surface',
    ...
    { 
      ...
      iconBg: 'bg-cream0 dark:bg-surface',
      color: 'bg-cream0 dark:bg-primary',
      slug: 'crm-and-tools'
    },
    { 
      ...
      iconBg: 'bg-yellow-500 dark:bg-surface',
    ...
    { 
      ...
      iconBg: 'bg-red-500 dark:bg-surface',
  ...
  return (
    <div className="p-6 bg-gradient-to-b from-cream to-gray-100 dark:from-surface dark:to-background transition-colors duration-300 rounded-lg">
