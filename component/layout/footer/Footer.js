...
const Footer = () => {
  ...
  const companyHighlights = [
    { icon: Award, value: '14+ Years', label: 'Digital excellence' },
          ...
          <div className="sm:col-span-2 lg:col-span-4 space-y-5">
            ...
            <div className="flex flex-wrap gap-2.5">
              {socials.map(({ icon: Icon, href, label, external }) => (
                <Link
                  ...
                  aria-label={label}
                  className={`flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 bg-white/5 text-white/60 transition-all duration-300 hover:border-accent/40 hover:text-accent hover:bg-white/10 hover:-translate-y-0.5 ${
                    label === 'WhatsApp' ? 'hover:text-[#25D366] hover:border-[#25D366]/40' : ''
                  }`}
                  {...(external ? {} : {})}
                >
                  <Icon size={16} />
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-1">
              {companyHighlights.map(({ icon: Icon, value, label }) => (
                ...
                >
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-primary/20 ring-1 ring-primary/25">
