    ...
    <section className="py-16 sm:py-20 bg-white dark:bg-gray-950">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-14">
          <h6 className="text-2xl sm:text-3xl md:text-4xl font-semibold text-heading">
            How We Start
          </h6>
        ...
        <div className="grid grid-cols-1 md:grid-cols-[1fr_auto_1fr_auto_1fr] items-center gap-6">
          {steps.map((step, index) => (
            <React.Fragment key={index}>
              {/* Step Card */}
              <div
                className="group bg-background border border-gray-200 dark:border-gray-800
              rounded-2xl p-6 sm:p-8 hover:shadow-xl hover:-translate-y-1 transition-all duration-300"
              >
                {/* Step Number */}
                <div
                  className="w-10 h-10 flex items-center justify-center rounded-full
                bg-primary text-white font-semibold mb-4"
                >
                  {index + 1}
                </div>
                ...
                {/* Hover Accent */}
                <div className="mt-5 w-10 group-hover:w-20 h-1 bg-primary transition-all duration-300"></div>
              </div>
              ...
              {index < steps.length - 1 && (
                <>
                  ...
                  <div className="hidden md:flex justify-center">
                    <FaArrowRight className="text-3xl text-primary dark:text-accent opacity-70" />
                  </div>

                  {/* Mobile Arrow (Down) */}
                  <div className="flex md:hidden justify-center">
                    <FaArrowDown className="text-2xl text-primary dark:text-accent opacity-70" />
                  </div>
                </>
              )}
            </React.Fragment>
        ...
        <div className="mt-14 text-center max-w-3xl mx-auto">
          <p className="text-body text-sm sm:text-base md:text-lg leading-7">
