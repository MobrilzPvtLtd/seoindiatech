...
const NewFooter = () => {
  ...
  const socials = [
    ...
    { icon: FaFacebook, href: 'https://www.facebook.com/profile.php?id=61582651577922', label: 'Facebook', hover: 'hover:text-accent' },
    { icon: FaLinkedin, href: 'https://www.linkedin.com/company/seo-india-tech/', label: 'LinkedIn', hover: 'hover:text-accent' },
  ]
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
              ...
              {services.map((service) => (
                <li key={service.slug}>
                  <Link
                    ...
                    className="flex items-center text-muted hover:text-white transition-colors duration-200 text-sm"
                  >
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4 text-sm">
              <p className="flex items-start space-x-3 text-muted">
            ...
            <div className="pt-1 space-y-3">
              <button
                ...
                className="w-full bg-white text-gray-900 px-6 py-3 rounded-full font-semibold text-sm transition-all duration-300 flex items-center justify-center space-x-2 hover:bg-background hover:scale-[1.02] active:scale-95"
              >
          ...
          {locations.map((city, i) => (
            <React.Fragment key={city}>
              <Link
                ...
                className="text-accent font-medium hover:text-accent hover:underline"
              >
                {city}
              </Link>
      ...
      <div className="relative border-t border-white/10 py-4">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row justify-between items-center gap-4">
          <div className="flex flex-col md:flex-row items-center md:space-x-6">
            <p className="text-muted text-sm">&copy; {currentYear} SEO India Tech - All rights reserved</p>
            <div className="flex space-x-4 mt-2 md:mt-0">
              <Link href="/privacy-policy" className="text-sm text-muted hover:text-accent transition-colors">Privacy Policy</Link>
