      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-blue-100 dark:border-blue-800/50 shadow-xl p-8 md:p-12 lg:p-16">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-4xl mx-auto mt-8 mb-12">
            {[
              { title: "Strategic Content", desc: "Data-driven content planning" },
              { title: "Engaging Storytelling", desc: "Memorable brand narratives" },
              { title: "Conversion Focused", desc: "Content that drives action" }
            ].map((feature, index) => (
          ...
          <div className='flex flex-col sm:flex-row justify-center gap-4 mt-8'>
            <button className='bg-blue-700 hover:bg-blue-800 dark:bg-blue-600 dark:hover:bg-blue-700 text-white px-6 py-3 rounded-lg text-base font-medium transition flex items-center justify-center group shadow-lg'>
              Get Started Now
              <ArrowRight className="ml-2 w-5 h-5 transition-transform duration-300 group-hover:translate-x-1" />
            </button>
            
            <button className='bg-transparent text-blue-700 dark:text-blue-400 border border-blue-200 dark:border-blue-700 hover:bg-blue-50 dark:hover:bg-blue-900/30 px-6 py-3 rounded-lg text-base font-medium transition'>
          ...
          <div className="hidden mt-12 text-center">
            <p className="text-gray-500 dark:text-gray-400 text-sm uppercase tracking-wider mb-4">Trusted by leading brands</p>
            <div className="flex flex-wrap justify-center gap-8 opacity-60">
              {['Microsoft', 'Adobe', 'Shopify', 'Slack', 'Spotify'].map((brand, index) => (
                <div key={index} className="text-gray-400 dark:text-gray-500 font-semibold">{brand}</div>
              ))}
            </div>
          </div>
        </div>
