...
const WorkFlowService = () => {
  const workflowSteps = [
    {
      id: 1,
      icon: <ClipboardList className="w-6 h-6 text-primary dark:text-accent" />,
      title: "Define",
    ...
    // {
    //   id: 3,
    //   icon: <Eye className="w-6 h-6 text-primary dark:text-accent" />,
    //   title: "Provide",
      ...
      <div className="max-w-5xl mx-auto relative z-10">
        <div className="bg-card dark:bg-card shadow-xl rounded-2xl overflow-hidden">
          ...
          <div className="p-6 md:p-10">
              ...
              {workflowSteps.map((step, index) => (
                ...
                >
                  ...
                  {index < workflowSteps.length - 1 && (
                    <div className="absolute left-6 top-16 bottom-0 w-0.5 bg-primary/15 dark:bg-primary/20"></div>
                  )}

                  <div className="flex gap-6">
                    {/* Step number and icon */}
                    <div className="flex-shrink-0 relative z-10">
                      <div className="flex items-center justify-center w-12 h-12 bg-primary/10 dark:bg-primary/20 rounded-full border-2 border-primary/30 dark:border-primary/30">
                        {step.icon}
                      </div>
                    ...
                    <div className="flex-grow">
                      <div className="bg-gray-50 dark:bg-card/80 border border-border dark:border-border rounded-xl p-6 shadow-sm">
                        <h3 className="font-semibold text-xl mb-2">
                           <span className="text-body font-normal">{step.text}</span>
                        </h3>
                        {/* <p className="mt-3 text-muted pl-0 md:pl-6 border-l-2 border-primary/30 dark:border-primary/40">
                          {step.detail}
                        </p> */}
                      </div>
                    </div>
            ...
            {/* Call to action */}
            <div className="hidden mt-12 pt-8 border-t border-border flex flex-col md:flex-row items-center justify-between gap-6">
              <div>
