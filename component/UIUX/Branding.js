    ...
    <section className='bg-blue-950 dark:bg-gray-900 py-16 md:py-24 px-6 relative overflow-hidden'>
      ...
      <div className="absolute inset-0 pointer-events-none">
        <div className="absolute top-0 right-0 opacity-20">
          <Star className="w-24 h-24 text-yellow-300 dark:text-yellow-500" fill="currentColor" />
        </div>
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="flex flex-col items-center">
          ...
          <div className="hidden mt-16">
            <p className="text-blue-200 dark:text-blue-100 text-sm font-medium uppercase tracking-wider mb-6 text-center">Trusted by leading brands</p>
            <div className="flex flex-wrap justify-center gap-8 md:gap-12">
              {["Google", "Microsoft", "Amazon", "Adobe", "Shopify"].map((company, index) => (
                <div key={index} className="text-white/40 dark:text-white/30 font-bold text-lg md:text-xl">
                  {company}
                </div>
              ))}
            </div>
